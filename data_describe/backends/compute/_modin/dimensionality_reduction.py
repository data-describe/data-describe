...
def compute_run_pca(data, n_components, column_names):
    ...
    return _compat.modin.DataFrame(reduc, columns=column_names), pca
...
def compute_run_ipca(data, n_components, column_names):
    ...
    return _compat.modin.DataFrame(reduc, columns=column_names), ipca
...
def compute_run_tsne(data, n_components, apply_tsvd):
    ...
    return _compat.modin.DataFrame(reduc, columns=["ts1", "ts2"]), tsne
...
def compute_run_tsvd(data, n_components, column_names):
    ...
    return _compat.modin.DataFrame(reduc, columns=column_names), t_svd
