...
@_context_manager
def cluster(df, method='KMeans', target=None, interactive=True, elbow=False, dim_method='pca',
            analysis='silhouette_score', return_value=None, kwargs=None, context=None):
    ...
    data = pd.DataFrame(scaler.fit_transform(num_df), index=num_df.index, columns=num_df.columns).dropna()
    ...
    if dim_method == "tsne":
        ...
        reduc_df, truncator = dim_reduc(data=target_df, n_components=2, dim_method=dim_method)

    if method == 'KMeans':
        viz, reduc_df = kmeans_cluster(data=data, reduc_df=reduc_df, target=target, analysis=analysis,
                                       interactive=interactive, elbow=elbow, truncator=truncator,
                                       kwargs=kwargs, context=context)
        df['cluster'] = reduc_df['cluster']

    elif method == 'HDBSCAN':
        viz, reduc_df = hdbscan_cluster(data=data, reduc_df=reduc_df, interactive=interactive,
                                        truncator=truncator, kwargs=kwargs, context=context)
        df['cluster'] = reduc_df['cluster']
...
@_context_manager
def kmeans_cluster(data, reduc_df, analysis='silhouette_score', n_clusters=None, cluster_min=2, cluster_max=20,
                   interactive=True, truncator=None, target=None, elbow=False, kwargs=None, context=None):
    ...
    if n_clusters is None:
        n_clusters, cluster_range, scores = find_clusters(data=data, cluster_min=cluster_min, cluster_max=cluster_max,
    ...
    if interactive is False:
        return static_plot(data=reduc_df, method='KMeans', x=reduc_df.columns[0], y=reduc_df.columns[1],
                           truncator=truncator), reduc_df
    else:
        return interactive_plot(df=reduc_df, method='KMeans', x=reduc_df.columns[0], y=reduc_df.columns[1],
                                color='cluster', truncator=truncator), reduc_df


def find_clusters(data, cluster_min, cluster_max, analysis='silhouette_score', target=None, kwargs=None):
...
def hdbscan_cluster(data, reduc_df, truncator=None, interactive=True, min_cluster_size=15, kwargs=None, context=None):
    ...
    if interactive is False:
        return static_plot(data=reduc_df, method='HDBSCAN', x=reduc_df.columns[0], y=reduc_df.columns[1],
                           truncator=truncator, context=context), reduc_df
    else:
        return interactive_plot(df=reduc_df, method='HDBSCAN', x=reduc_df.columns[0], y=reduc_df.columns[1],
                                color='cluster', truncator=truncator, context=context), reduc_df
...
def interactive_plot(df, x, y, method, color, truncator=None, context=None):
    ...
    try:
        ...
        df_copy['{} ({}% variance explained)'.format(x, str(round(truncator.explained_variance_ratio_[0]*100, 2)))] = df[x]
        df_copy['{} ({}% variance explained)'.format(y, str(round(truncator.explained_variance_ratio_[1]*100, 2)))] = df[y]
        x = df_copy.columns[-2]
        y = df_copy.columns[-1]
    except:
        pass
    ...
    layout = dict(
                  ...
                  title={"text": '{} Cluster'.format(method),
                         "font": {"size": 25}})
...
def static_plot(data, x, y, method, truncator=None, context=None):
    ...
    fig = sns.lmplot(data=data, x=x, y=y, hue='cluster', palette=p, fit_reg=False, height=context.fig_height,
                     aspect=context.fig_width/context.fig_height, legend=False)
    sns.set_context("talk")
    plt.legend(bbox_to_anchor=(1.1, 1), loc='upper right', ncol=1, title='Cluster')
    plt.title(method + ' Cluster')
    try:
        plt.xlabel(x + ' ' + '({}% variance explained)'.format(str(round(truncator.explained_variance_ratio_[0]*100, 2))))
        plt.ylabel(y + ' ' + '({}% variance explained)'.format(str(round(truncator.explained_variance_ratio_[1]*100, 2))))
    except:
        plt.xlabel(x)
        plt.xlabel(y)
    return fig
