...
def preprocess(data, target, impute='simple', encode='label'):
    ...
    cat = data[[c for c in data.columns if not c in num.columns]]
    if cat.shape[1] > 0:
        cat = cat.astype(str)
        cat = cat.fillna("")
        if encode == 'label':
    ...
    if num.shape[1] > 0 and cat.shape[1] > 0:
        X = pd.DataFrame(np.concatenate([x_num, x_cat], axis=1), columns=list(num.columns.values) + list(cat.columns))
    elif num.shape[1] > 0:
