...
def test_scagnostic_class_np(np_data):
    assert isinstance(mw.metrics.bivariate.Scagnostics(np_data), mw.metrics.bivariate.Scagnostics)


def test_str_data_type():
    assert isinstance(mw.metrics.bivariate.Scagnostics('data/er_data.csv'), mw.metrics.bivariate.Scagnostics)
