...
def test_kmeans_default(kmeans_default):
    ...
    assert is_dataframe(cl.input_data), "Input data is not a data frame"
    assert hasattr(cl, "scaled_data"), "Widget does not have standardized data"
    assert is_dataframe(cl.scaled_data), "Scaled data is not a data frame"
    assert hasattr(cl, "viz_data"), "Widget does not have visualization (reduced) data"
    assert is_dataframe(cl.viz_data), "Viz data is not a data frame"
