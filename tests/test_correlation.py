...
def test_corr_values(data):
    corr_values = dd.correlation_matrix(data, return_values=True)
...
def test_categorical_data_only(data):
    ...
    corr_values = dd.correlation_matrix(cat_data, categorical=True, return_values=True)
...
def test_categorical_data_only_but_specified_numeric(data):
    ...
    with pytest.raises(ValueError):
        dd.correlation_matrix(cat_data, return_values=True)


def test_numeric_data_only_but_specified_categorical(data):
    num_data = data.select_dtypes(["number"])
    with pytest.warns(UserWarning):
        dd.correlation_matrix(num_data, categorical=True, return_values=True)
