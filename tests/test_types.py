import pandas as pd
from mwdata.type.autotype import guess_dtypes, guess_series_dtypes, meta_features, get_class_instance_by_name, \
    cast_dtypes, select_dtypes
from mwdata.type.dtypes import BaseType, StringType, IntegerType, DecimalType, CategoryType, DateTimeType, \
    ReferenceType, BoolType, NumericType
import datetime
import decimal
import six
import pytest
...
def load_data():
    ...
    df = df.sample(5, axis=1, random_state=2)
    return df


def test_guess_types(load_data):
    assert guess_dtypes(load_data) == {'glimepiride.pioglitazone': 'Boolean',
...
def guess_series_type(load_data):
    assert guess_series_type(load_data['diag_2_desc'].iloc[5:15]) == {'diag_2_desc': 'String'}


def test_select_dtypes(load_data):
    assert select_dtypes(load_data, types=['String'], dtypes={'diag_2_desc': 'String'}).columns == ['diag_2_desc']


def test_cast_types(load_data):
    df = cast_dtypes(load_data, dtypes={'readmitted': 'Category'}, exclude=['glimepiride.pioglitazone'])
    assert str(df.dtypes.values) == "[dtype('O') Int64Dtype() dtype('O') dtype('O') dtype('O')]"
    with pytest.raises(ValueError):
